...
public interface SanPhamChiTietRepository extends JpaRepository<SanPhamChiTiet, Long> {
    ...
    @Query("select s from SanPhamChiTiet s where s.sp.tenSP like ?1 or s.sp.maSP like ?1 or s.ms.ten like ?1")
    public Page<SanPhamChiTiet> search(String search, Pageable pageable);

    @Query("select s from SanPhamChiTiet s where s.size = ?1")
    public Page<SanPhamChiTiet> findBySize(String search, Pageable pageable);
}
